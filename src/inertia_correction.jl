...
function inertia_correction_slack!(s::Solver; restoration=false)
    ...
    if s.inertia_slack.z != 0
        s.opts.verbose ? (println("$(s.inertia_slack.z) zero eigen values")) : nothing
        s.δc = s.opts.δc*s.μ^s.opts.κc
    end
    ...
    while !inertia_slack(s)
        ...
        if inertia_slack(s)
            s.opts.verbose ? println("inertia (corrected)-> n: $(s.inertia_slack.n), m: $(s.inertia_slack.m), z: $(s.inertia_slack.z)") : nothing
            break
        else
            # IC-5
            if s.δw_last == 0
        ...
        if s.δw > s.opts.δw_max
            if s.opts.verbose
                println("n: $(s.inertia_slack.n), m: $(s.inertia_slack.m), z: $(s.inertia_slack.z)")
                println("s.δw: $(s.δw)")
            end
            # skp backtracking line search and go to restoration phase
            # TODO: handle inertia correction failure gracefully
