function inertia_correction!(H,s::Solver,restoration=false)
    n = -1
    m = -1
    z = -1
    ...
    try
        ...
        z = sum(abs.(LDL.D) .< s.opts.ϵ_mach)
        # if kkt_solve == :symmetric
        #     δ = Diagonal([s.δw*ones(s.n);-s.δc*ones(s.m)])
        # elseif :unreduced
        #     δ = Diagonal([s.δw*ones(s.n);-s.δc*ones(s.m);zeros(s.nL+s.nU)])
        # else
        #     error("kkt solve error in inertia correction")
        # end
        # e = eigen(Array(H + δ))
        # n = sum(e.values .>= s.opts.ϵ_mach)
        # m = sum(e.values .<= s.opts.ϵ_mach)
        # z = s.n+s.m - n - m
    catch
    ...
    # s.δw = s.δc
    #
    # println("rank: $(rank(H + Diagonal([s.δw*ones(s.n);-s.δc*ones(s.m)])))")
    # println("δw: $(s.δw)")
    # println("δc: $(s.δc)")
