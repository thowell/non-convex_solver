...
function α_min!(s::Solver)
    s.α_min = α_min(s.d_primal,s.θ,s.∇φ,s.θ_min,s.opts.δ,s.opts.γα,s.opts.γθ,s.opts.γφ,s.opts.sθ,s.opts.sφ)
    return nothing
end
...
function α_max!(s::Solver)
    ...
    if s.mI != 0
        while !fraction_to_boundary_slack_bnds(s.s,s.sL,s.ds,s.α_max,s.τ)
            s.α_max *= 0.5
        end
    end
    s.α = copy(s.α_max)
...
function αz_max!(s::Solver)
    ...
    if s.mI != 0
        while !fraction_to_boundary(s.zS,s.dzS,s.αz,s.τ)
            s.αz *= 0.5
        end
    end
...
function switching_condition(s::Solver)
    return switching_condition(s.∇φ,s.d_primal,s.α,s.opts.sφ,s.opts.δ,s.θ,s.opts.sθ)
end
...
armijo(s::Solver) = armijo(s.φ⁺,s.φ,s.opts.ηφ,s.α,s.∇φ,s.d_primal,
    s.opts.ϵ_mach)
