...
function unpack(x)
    ...
    sfc = nc == 1 ? x[nq+nu+nc+nβ+nc+nc+nβ+nc] : x[nq+nu+nc+nβ+nc+nc+nβ .+ (1:nc)]
...
x0 = zeros(T*nx)
for t = 1:T
    x0[(t-1)*nx .+ (1:nx)] .= [Q0[t+2];y0;β0;ψ0;η0;ϕ(Q0[t+2]);(0.5*y0) - sum(β0)]
end
...
Q = [qpp,qp]
for t = 1:T
    q,u,y,β,ψ,η,sϕ,sfc = unpack(s.s.x[(t-1)*nx .+ (1:nx)])
    push!(Q,q)
end
Q[end]
Q_nl[end]
...
function visualize!(vis,qnl,q; r=0.25)

    setobject!(vis["particle_nl"], HyperRectangle(Vec(0,0,0),Vec(2r,2r,2r)),MeshPhongMaterial(color=RGBA(0, 1, 0, 1.0)))
    setobject!(vis["particle"], HyperRectangle(Vec(0,0,0),Vec(2r,2r,2r)),MeshPhongMaterial(color=RGBA(1, 0, 0, 1.0)))

    anim = MeshCat.Animation(convert(Int,floor(1/Δt)))

    for t = 1:length(q)
        MeshCat.atframe(anim,t) do
            settransform!(vis["particle_nl"], Translation(qnl[t][1:3]...))
...
settransform!(vis["/Cameras/default"], compose(Translation(1.5, 1., -1),LinearMap(RotZ(pi/2)*RotY(-pi/1.5)*RotX(pi))))
