...
function unpack(x)
    ...
    rϕ = nc == 1 ? x[nq+nu+nc+nβ+nc+nc+nc+nc] : x[nq+nu+nc+nβ+nc+nc+nc .+ (1:nc)]
    rfc = nc == 1 ? x[nq+nu+nc+nβ+nc+nc+nc+nc+nc] : x[nq+nu+nc+nβ+nc+nc+nc+nc .+ (1:nc)]
...
function f_func(z)
    _sum = 0.
    for t = 1:T
        q,u,y,β,ψ,sϕ,sfc,rϕ,rfc = unpack(z[(t-1)*nx .+ (1:nx)])
...
c_al_idx_t[1:nq+nβ+nc+nc+nc+nc] .= 0
...
x0 = zeros(T*nx)
for t = 1:T
    x0[(t-1)*nx .+ (1:nx)] .= [Q0[t+2];u0;y0;β0;ψ0;ϕ(model,Q0[t+2]);(model.μ*y0)^2 - β0'*β0;r0;r0]
end
